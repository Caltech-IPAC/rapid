...
- `rapidpipe run timings <run> [--stage S] [--json]` prints one row per attempt (stage, unit, attempt, disposition, `queue_s`/`exec_s`/`reconcile_lag_s` derived from Batch's own job timestamps -- `reconcile` copies `createdAt`/`startedAt`/`stoppedAt` into `execution_records.scheduler_metadata` under a `"batch"` key -- plus `over_30m`), then a per-stage summary (count, median, p90, max of `exec_s`, and how many exceeded 30 minutes). `fetch_s`/`body_s`/`publish_s` always print `-`: the stage's own execution record carries a `timing` key with these (`RAPIDPIPE_PROFILE`'s neighbour, direction/logging-timing), but `record_attempt_result` only ever persists the named `execution_records` columns, so they never reach a column to read back; a local run or an attempt Batch never resolved also prints `-` throughout. Read-only, data on stdout; exits 0, or 64 for an unknown run.
