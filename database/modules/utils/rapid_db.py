...
########################################################################################################
    ...
    def get_association_claim_position(self,association_set=None,lane=None):

        '''
        The live lane's association watermark as (proc_date, field), or None
        when the ordering schema is absent (conformance rule 19, brief F2).

        The watermark is the last ACCEPTED crossmatch unit's position in the
        canonical (proc_date, field) order — the frontier the claim path
        yields the successor of. (None, None) is the origin: the row exists
        and nothing has been accepted in this set yet.

        RETURNS None WHEN `association_watermarks` IS NOT THERE, and that is
        not an error. The table ships as DRAFT migration 049 under
        `migrations-draft/` and is not in the authoritative stream until
        `rapid_systems` adopts it, so every database in the fleet that has not
        taken the draft answers None here and gathering degrades to its
        previous unordered behaviour explicitly. Probed through `to_regclass`
        rather than by running the query and interpreting the failure: "this
        schema is not deployed" and "this query is wrong" are two facts that
        must never be conflated, because conflating them turns a broken query
        into a silent degradation.

        `association_set` defaults to the live prompt set through the one
        well-known-row lookup (`derived.live_association_set()`); no caller
        and no query here hard-codes it.
        '''

        self.exit_code = 0

        probe = "select to_regclass('public.association_watermarks');"

        try:
            self.cur.execute(probe)
            row = self.cur.fetchone()
            if row is None or row[0] is None:
                return None

        except (Exception, psycopg2.DatabaseError) as error:
            print('*** Error probing for association_watermarks: {}; skipping...'.format(error))
            self.exit_code = 67
            return

        query = "select w.watermark_proc_date, w.watermark_field " +\
                "from association_watermarks w " +\
                "where w.association_set = coalesce(%s, derived.live_association_set()) " +\
                "and w.lane = coalesce(%s, 0);"

        params = (association_set, lane)
        ...
        try:
            ...
            print('*** Error reading the association watermark: {}; skipping...'.format(error))
            self.exit_code = 67
            return
...
########################################################################################################
    ...
    def get_earliest_unaccepted_crossmatch_date(self,association_set=None):

        '''
        The earliest processing date that still owes crossmatch work — the
        CROSS-DATE half of the ordering gate (conformance rule 19, brief F2).

        Gathering is invoked once per processing date, so the watermark alone
        cannot tell a pass for date d2 that d1 is still in retry. This
        answers exactly that: among the dates with science jobs, the earliest
        one at or after the watermark whose fields are not all accepted.
        A later date is not claimable while this returns a date before it —
        which is the "a later observation's association cannot run ahead of
        an earlier one still in retry" property (§2.5), the whole reason the
        ordering exists.

        "Owes work" is read off the same fact the resubmission gate reads —
        a field with no pending-or-successful crossmatch attempt — so the two
        gates cannot disagree about what counts as done. A field whose only
        attempts FAILED is owed, which is what makes a failed-and-retryable
        unit block its successors rather than being stepped over.

        Returns None when the ordering schema is absent, or when nothing is
        owed at all; both readings make the same decision in the caller.
        '''
        ...
        # Dates at or after the watermark's own date that still have a field
        # without a pending-or-successful crossmatch attempt. The watermark
        # bounds the scan: everything before it is accepted by definition, so
        # the query never walks the whole attempts corpus.
        query = "with wm as (" +\
                ...
                "  and w.lane = 0" +\
                "), science as (" +\
                "  select distinct to_char(la.processing_date, 'YYYYMMDD') as pd, la.field " +\
                "  from Attempts la " +\
                "  join logical_jobs lj on lj.logical_job_id = la.logical_job_id " +\
                "  where lj.job_type = %s and la.field is not null" +\
                "), accepted as (" +\
                "  select distinct to_char(la.processing_date, 'YYYYMMDD') as pd, la.field " +\
                "  from Attempts la " +\
                "  join logical_jobs lj on lj.logical_job_id = la.logical_job_id " +\
        ...
        from submission.routes import JOB_TYPE_SCIENCE, JOB_TYPE_CROSSMATCH

        params = (association_set, JOB_TYPE_SCIENCE, JOB_TYPE_CROSSMATCH)
        ...
        try:
            ...
            print('*** Error reading the earliest unaccepted crossmatch date: {}; skipping...'.format(error))
            self.exit_code = 67
            return
