...
########################################################################################################
    ...
    def get_data_classes_for_l2files(self,rids):
        ...
        '''
        The data class(es) recorded for the admission manifests covering
        these L2 files — the provenance chain's read side.

        Migration 090 records `data_class` on `admission_manifests`, where
        it is known: one manifest is one ingest run of one input set, which
        is the grain at which substrate and injection are fixed. A work
        unit's class is INHERITED from the manifests covering its inputs,
        joined `admission_l2files.rid -> admission_l2files.manifest_id ->
        admission_manifests.manifest_id`.

        RETURNS THE DISTINCT SET, NOT ONE VALUE, and that is the point: a
        unit whose inputs span manifests of different classes must take the
        most restrictive of them (operations.md, "a mixed derivation takes
        the most restrictive class of any input"), and that combination is
        `submission.data_class.most_restrictive`'s job, not this query's.
        Returning one row here — a `max()`, a `LIMIT 1` — would silently
        pick a class by spelling or by physical row order and would be
        wrong for exactly the mixed case the rule exists for.

        NULLs are excluded rather than returned: a manifest ingested before
        090 carries no class, and it contributes no evidence about what the
        unit is. An empty result means nothing covering these inputs knows
        the class, which the caller resolves to no class at all rather than
        to a guess.

        A rid with no admission row at all simply contributes nothing —
        this is a legacy-registered L2 file, and the same empty result
        follows.
        '''
        ...
        try:
            ...
            print('*** Error getting data classes for rids {}: {}; skipping...'.format(rid_list,error))
            self.exit_code = 67
            return
