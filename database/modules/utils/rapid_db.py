...
########################################################################################################
    ...
    def add_psf(self,fid,sca,status,filename,checksum,
        ...
        '''
        Add record in PSFs database table (dev 70565e0c, Russ Laher; ported).

        The row lands at vbest = 0 and version = max + 1 for its (fid, sca):
        insert never promotes (design/catalog.md, Promotion).  Promotion is
        update_psf's, as a separate and deliberate call, so registering a new
        PSF version cannot change which PSF the next unit runs with unless
        the caller says so.

        attempt_id and registered_record_sequence are the ATTEMPT IDENTITY of
        a registration inserting this row; migration 026 threads them into
        addPSF exactly as 018 did for addRefImage (see add_refimage for the
        contract).  Both default to None so a standalone caller -- the PSF
        registration script -- behaves exactly as before: mint a version,
        insert, return.
        '''
...
########################################################################################################
    ...
    def update_psf(self,psfid,filename,checksum,status,version):
        ...
        '''
        Update record in PSFs database table (dev 70565e0c, Russ Laher; ported).

        This is the promotion: updatePSF demotes the current best for the
        row's (fid, sca) and promotes this version to vbest = 1, refusing to
        demote an operator-pinned vbest = 2 (design/catalog.md, Promotion).
        '''
