...
| `test_ned_match.py` | NED cross-match: the host-candidate selection (`select_host_candidates` and its knobs), slice building and null handling, sep/PA geometry vs the KONA matcher, radius/nearest-3 selection, batch ≡ single, the per-chip bounding cone, the three nedMatches states end-to-end over the fake chip with an injected reader (incl. the failure cache), and the astroquery column mapping over a synthetic masked table. No network. |
