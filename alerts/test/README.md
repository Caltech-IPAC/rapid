...
| `test_ned_match.py` | Also the `Hp6NedReader` tests (local order-6 NED copy: cone reads, pixel-edge cones, empty-vs-null, the pixel cache, and a full alert over the fake chip whose nedMatches come from a synthetic store). NED cross-match: the host-candidate selection (`select_host_candidates` and its knobs), slice building and null handling, sep/PA geometry vs the KONA matcher, radius/nearest-3 selection, batch ≡ single, the per-chip bounding cone, the three nedMatches states end-to-end over the fake chip with an injected reader (incl. the failure cache), and the astroquery column mapping over a synthetic masked table. No network. |
| `test_ned_catalog.py` | `alerts/ned_catalog.py`, the NED HATS collection tool: streaming `mirror` over a local HTTP server into a local store with md5 verification (and rejection of a corrupted file), `verify`, `repartition` into order-6 files (correct pixel, sorted, `ptype` "" → null, idempotent done markers), `manifest`, and the CLI. No S3, no network. |
| `test_lvs_match.py` | NED-LVS cross-match: `build_lvscat`/`match_lvscat` (every LVS column reaching the record, nulls → None, quality flags stay bool, nearest-3 within 30″, batch ≡ single), the three lvsMatches states end-to-end over the fake chip, the Avro round trip, the join to nedMatches by `prefName`, and `LvsReader` over a synthetic `lvs/nedlvs.parquet` (cone reads, one load per process, missing-table refusal). No S3, no network. |
| `test_stage.py` | `pipeline/produceAlertsForProcDate.py`, the launcher-side VPO stage: `[ALERTS]` settings (and the refusal to start with `publish_to_kafka = True`), the processing-date → chips lookup over a stub `RAPIDDB`, one chip's archive + `BatchStats`, the worker loop recording a failed chip and continuing, the multi-process path (fork only), and the run summary's exit codes. No DB, no S3. |
| `test_live_db.py` | Also holds `test_live_associations_reference_existing_objects`, a **strict xfail** pinning a known upstream bug (2026-09-18): in `merges_4715492`, 36% of rows point at aids missing from `astroobjects_4715492` and 63,580 sids have several merges rows. It reports as an expected failure with the live counts until the cross-match/statistics tables are repaired; when they are, strict xfail fails the run, which is the cue to delete the marker. Live-database integration: the pixel-convention sentinel, the end-to-end production round trip (real alert → Avro → decode, cutouts FITS-verified), the `--kona-file` wiring against a real alert, and the reference-catalog match against the real mosaic catalog. **Skips** without DB access (see below). |
