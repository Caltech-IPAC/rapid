...
class AlertDataProvider:
    ...
    def __init__(self, db: Any, diff_flavor: str = "sfft",
                 kona_lookup: Any = None, refcat: bool = True) -> None:
        """
        Parameters
        ----------
    ...
    def _chip_images(self, pid: int) -> dict[str, LoadedImage]:
        ...
        # _stage() raises on an S3 download failure; this comprehension then
        # aborts before the chip is cached.
        images = {key: load_fits_image(self._stage(f"{job_dir}/{name}"))
                  for key, name in names.items()}
        # A file that staged but did not load (missing local file, no image
        # HDU, corrupt) comes back as (None, None) -- also abort the chip.
