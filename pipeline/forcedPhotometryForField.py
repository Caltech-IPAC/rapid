...
def add_simulated_point_source_to_difference_image(diffimg_filename,
    ...
    i = round(y_zerobased.item())
    j = round(x_zerobased.item())
...
if __name__ == '__main__':
    ...
    for record in records:
        ...
        if percent_overlap_area > minimum_percent_overlap_area:
            ...
            # PSF per detector (Jacob Jencson, dev 747d15c4). SCAID is used rather than
            # SCA because real filenames contain "SCA". A no-op for templates without
            # the token, e.g. refimage_psf_fidFID.fits. This is the only place on smdc
            # that still reads the [JOB_PARAMS] refimage_psf_filename template; the
            # science stage takes its PSF from the unit's psf_uri fact.
...
#---------------------------------------------------------------------
    ...
    for ra,dec,reqid in zip(ra_list,dec_list,reqid_list):
        ...
        fh_lc.write(f"# ------------------------------------------------------------------\n")
        fh_lc.write(f"# RAPID Forced-Photometry Lightcurve extracted from Difference Images\n")
        ...
        fh_lc.write(f"# ------------------------------------------------------------------\n")
        fh_lc.write(f"# Value of -99999 in table below means no calculation is possible.\n")
        fh_lc.write(f"# ------------------------------------------------------------------\n")
