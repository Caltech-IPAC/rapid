...
if __name__ == '__main__':
    ...
    for ra,dec,reqid in zip(ra_list,dec_list,reqid_list):
        ...
        fh_lc.write(f"# ------------------------------------------------------------------\n")
        fh_lc.write(f"# RAPID Forced-photometry Lightcurve on Difference Images\n")
        fh_lc.write(f"# Generated by {swname}. v{swvers} on {proc_pt_datetime_started}\n")
        ...
        fh_lc.write(f"# psfflux = Forced difference image PSF-fit flux [DN/s]\n")
        ...
        fh_lc.write(f"# psfchi2 = Reduced chi-square in PSF-fit\n")
        fh_lc.write(f"# aperflux = Forced difference image aperture flux using a fixed {apdiam}-pixel diameter aperture [DN/s]\n")
        ...
        fh_lc.write(f"# aperturecorr = Aperture (curve-of-growth) correction factor applied to aperflux measurement\n")
        fh_lc.write(f"# procstatus = Per-epoch processing status codes (0 => normal); non-zero is abnormal (see documentation).\n")
        fh_lc.write(f"# ------------------------------------------------------------------\n")
        fh_lc.write(f"# Order of table columns as follows:\n")
