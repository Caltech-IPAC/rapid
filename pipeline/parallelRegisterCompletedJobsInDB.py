...
def run_single_core_job(jids,log_fnames,index_thread):
    ...
    '''
    Close out completed Jobs database records for a thread's share of jids.  Returns
    (n_registered,n_failed) counts so that a job which raises partway through cannot
    be logged and then forgotten.
    '''
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if num_cores == 1:
            n_registered,n_failed = run_single_core_job(jids,log_filenames,0)
        else:
            n_registered,n_failed = execute_parallel_processes(jids,log_filenames,num_cores)
