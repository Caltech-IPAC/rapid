...
swname = "pruneRedundantMerges.py"
swvers = "1.0"
...
def run_single_core_job(fields,index_thread):
    ...
    for index_field in range(nfields):
        ...
        fh.write(f"Elapsed time in seconds to remove redundant records from {merges_tablename} database table = {diff_time_benchmark}\n")
...
if __name__ == '__main__':
    ...
    AstroObjects for all fields (for which merges tables exist).
    '''
    ...
    end_time_benchmark = time.time()
    print(f"Elapsed time in seconds to delete all redundant merges =",
