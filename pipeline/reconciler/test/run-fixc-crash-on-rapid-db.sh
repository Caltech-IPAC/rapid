#!/usr/bin/env bash
# run-fixc-crash-on-rapid-db.sh — drive the crash-boundary probe against rapid-db.
#
...
#
# Usage: run-fixc-crash-on-rapid-db.sh [image-ref]
...
RUN_ID="fixc-crash-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/fixc-crash.XXXXXX.tar.gz")
...
aws s3 cp "$tarball" "s3://$BUCKET/$PREFIX/fixc-crash.tar.gz" --only-show-errors
...
aws s3 cp "s3://$BUCKET/$PREFIX/fixc-crash.tar.gz" "\$STAGE/fixc-crash.tar.gz" --only-show-errors
echo "$sha  \$STAGE/fixc-crash.tar.gz" > "\$STAGE/manifest.sha256"
sha256sum -c "\$STAGE/manifest.sha256" || { echo "!! checksum FAILED" >&2; exit 1; }
mkdir -p "\$STAGE/repo"
tar xzf "\$STAGE/fixc-crash.tar.gz" -C "\$STAGE/repo"
...
podman run --rm --entrypoint="" -v "\$STAGE/repo":/fixc:Z -w /fixc \\
  ...
  --network host \\
  "$IMAGE" python3.11 -m pipeline.reconciler.test.live_fixc_crash_boundary 2>&1 | tail -120
live=\${PIPESTATUS[0]}
...
params=$(mktemp "${TMPDIR:-/tmp}/fixc-crash-params.XXXXXX")
...
echo "FIXC-CRASH-VERIFICATION-OK"
