#!/usr/bin/env bash
# run-fixa-live-on-rapid-db.sh — drive the FixA live probe against rapid-db.
#
...
#
# Usage: run-fixa-live-on-rapid-db.sh [image-ref]
...
RUN_ID="fixa-live-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/fixa-live.XXXXXX.tar.gz")
...
aws s3 cp "$tarball" "s3://$BUCKET/$PREFIX/fixa-live.tar.gz" --only-show-errors
...
aws s3 cp "s3://$BUCKET/$PREFIX/fixa-live.tar.gz" "\$STAGE/fixa-live.tar.gz" --only-show-errors
echo "$sha  \$STAGE/fixa-live.tar.gz" > "\$STAGE/manifest.sha256"
sha256sum -c "\$STAGE/manifest.sha256" || { echo "!! checksum FAILED" >&2; exit 1; }
mkdir -p "\$STAGE/repo"
tar xzf "\$STAGE/fixa-live.tar.gz" -C "\$STAGE/repo"
...
podman run --rm --entrypoint="" -v "\$STAGE/repo":/fixa:Z -w /fixa \\
  ...
  --network host \\
  "$IMAGE" python3.11 -m pipeline.reconciler.test.live_fixa_probe 2>&1 | tail -70
live=\${PIPESTATUS[0]}
...
params=$(mktemp "${TMPDIR:-/tmp}/fixa-live-params.XXXXXX")
