#!/usr/bin/env bash
# run-fixa-on-rapid-admin.sh — run every suite the FixA protocol round
# touches, in-image on rapid-admin.
#
# Same shape and same reasons as W5's runner
# (pipeline/stages/test/run-w5-on-rapid-admin.sh) and W4's before it:
# psycopg2 is not installed on the laptop and would not be the right
...
#
# WHAT FIXA ADDS to the suites W5's runner already ran:
#
#   pipeline/reconciler/test — the reconciler: scheduler index derivation
#     and per-attempt statuses (#4), closure-record completeness and
#     sequence consistency (#14, #15), retention tag reads that
#     distinguish absence from error and closure failures that defer
#     rather than terminalize (#16), the work-capable health gate (#24).
#   pipeline/test/test_seams.py — the submission ORDER (#2): rows before
#     SubmitJob, backfill after, and a SubmitJob failure leaving
#     reconciliation cases rather than orphans.
#
# The suites are otherwise W5's, because the protocol fixes reach into
# every one of them: the attempt writer (#3, #10, #14), the entrypoint
# (#5, #6, #13, #17), the runtime (#9, #10), submission (#11, #12, #18).
#
# Usage: run-fixa-on-rapid-admin.sh [image-ref]
...
RUN_ID="fixa-tests-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/fixa-tests.XXXXXX.tar.gz")
...
aws s3 cp "$tarball" "s3://$BUCKET/$PREFIX/fixa-tests.tar.gz" --only-show-errors
...
aws s3 cp "s3://$BUCKET/$PREFIX/fixa-tests.tar.gz" "\$STAGE/fixa-tests.tar.gz" --only-show-errors
echo "$sha  \$STAGE/fixa-tests.tar.gz" > "\$STAGE/manifest.sha256"
sha256sum -c "\$STAGE/manifest.sha256" || { echo "!! checksum FAILED" >&2; exit 1; }
mkdir -p "\$STAGE/repo"
tar xzf "\$STAGE/fixa-tests.tar.gz" -C "\$STAGE/repo"

aws ecr get-login-password --region us-east-1 \\
  | podman login --username AWS --password-stdin ${ACCOUNT}.dkr.ecr.us-east-1.amazonaws.com >/dev/null

run() {
  podman run --rm --entrypoint="" -v "\$STAGE/repo":/fixa:Z -w /fixa "$IMAGE" "\$@"
}

echo
echo "=== FixA reconciler suite ==="
run python3.11 -m pytest pipeline/reconciler/test -q 2>&1 | tail -15
rec=\${PIPESTATUS[0]}
echo ">> reconciler suite exit code: \$rec"

echo
echo "=== FixA seams suite (submission order) ==="
...
params=$(mktemp "${TMPDIR:-/tmp}/fixa-params.XXXXXX")
