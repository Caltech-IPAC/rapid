...
def download_inputs(context) -> None:
    ...
    **NO DATABASE WRITE HAPPENS HERE ANY MORE** (conformance rule 8, brief
    C2). This function used to open with `_record_l2_available_milestone
    (context)`, which called `context.require_connection()` for the borrowed,
    attempt-long connection and then read, wrote and COMMITTED on it — inside
    the product-producing sequence that this same function continues into with
    `context.produce("science_image_gz", ...)` and both PSFs. That is the very
    split `pipeline.stages.context` documents against itself: database effects
    belong to post-DB job types ("the post-DB job types produce database state
    rather than S3 products, so they need that connection"), and a science
    stage is not one. Rule 8 says it from the worker's side: "Pixel/transform
    workers hold no database connection. They upload artifacts and one sealed,
    checksummed result manifest, then exit."

    The milestone now lands on the ACCEPTANCE side, inside the registration
    consumer's per-attempt transaction, where it commits atomically with the
    product rows and the watermark — see
    `pipeline.registration.consumer.record_l2_available`, which also records
    the timing-semantics change and the search establishing that nothing reads
    this milestone today. Nothing replaces the call here: the correct number
    of database statements in a product-producing science stage is zero.
    """
