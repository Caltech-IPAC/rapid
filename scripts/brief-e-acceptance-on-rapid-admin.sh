#!/usr/bin/env bash
#
# brief-e-acceptance-on-rapid-admin.sh — stage this branch and the
# authoritative migration stream to rapid-admin, and run brief E's acceptance
# there via SSM.
#
...
#
# Usage: brief-e-acceptance-on-rapid-admin.sh [postgres-image-ref]
...
# `<rapid-parent>/rapid-wt-e`, so rapid_systems is one level up beside it —
...
RUN_ID="brief-e-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/brief-e.XXXXXX.tar.gz")
...
# the tree by extension trips over them — brief E's fields-reader scanner did
...
sha256sum -c "\$STAGE/manifest.sha256" || { echo "BRIEF-E-CHECKSUM: FAIL exit=1"; exit 1; }
echo "BRIEF-E-CHECKSUM: PASS exit=0"
...
echo "BRIEF-E-STREAM-REV: $STREAM_REV"
echo "BRIEF-E-MIGRATIONS: \$(ls \$STAGE/db-migrations/0[0-9][0-9]-*.sql | wc -l) stream, \$(ls \$STAGE/repo/migrations-draft/0[0-9][0-9]-*.sql | wc -l) draft"

aws ecr get-login-password --region us-east-1 \\
  | podman login --username AWS --password-stdin ${ACCOUNT}.dkr.ecr.us-east-1.amazonaws.com >/dev/null 2>&1
podman pull "$IMAGE" >/dev/null 2>&1 || { echo "BRIEF-E-PULL: FAIL exit=1"; exit 1; }
echo "BRIEF-E-PULL: PASS exit=0"

"\$STAGE/repo/scripts/contract-brief-e-on-rapid-admin.sh" "\$STAGE" "$IMAGE" "${RUN_ID}"
rc=\$?
echo "BRIEF-E-RUNNER: exit=\$rc"
exit \$rc
REMOTE
)

b64=$(printf '%s' "$script" | base64 | tr -d '\n')
params=$(mktemp "${TMPDIR:-/tmp}/brief-e-params.XXXXXX")
printf '{"commands":["echo %s | base64 -d | bash"]}' "$b64" > "$params"

echo
echo ">> running brief E acceptance on rapid-admin via SSM..."
...
rm -f "$params"
echo ">> command id: $cmd"
echo "$cmd" > "${TMPDIR:-/tmp}/brief-e-last-command-id"
...
if [ "$status" = "Success" ]; then
    ...
    echo "BRIEF-E-ACCEPTANCE-OK"
    exit 0
fi
echo "!! brief E acceptance FAILED (status $status)" >&2
