#!/usr/bin/env bash
#
# brief-r-acceptance-on-rapid-admin.sh — stage this branch and the
# authoritative migration stream to rapid-admin, and run brief R's acceptance
# there via SSM.
#
...
#
# Derived from `brief-e-acceptance-on-rapid-admin.sh`, itself derived from
...
#
# Usage: brief-r-acceptance-on-rapid-admin.sh [postgres-image-ref]
...
# `<rapid-parent>/rapid-wt-r`, so rapid_systems is one level up beside it —
...
RUN_ID="brief-r-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/brief-r.XXXXXX.tar.gz")
...
#
# `.github` JOINS THE LIST for brief R. Acceptance 10 requires the workflow's
...
# the tree by extension trips over them — brief R's fields-reader scanner did
...
sha256sum -c "\$STAGE/manifest.sha256" || { echo "BRIEF-R-CHECKSUM: FAIL exit=1"; exit 1; }
echo "BRIEF-R-CHECKSUM: PASS exit=0"
...
echo "BRIEF-R-STREAM-REV: $STREAM_REV"
echo "BRIEF-R-MIGRATIONS: \$(ls \$STAGE/db-migrations/0[0-9][0-9]-*.sql | wc -l) stream, \$(ls \$STAGE/repo/migrations-draft/0[0-9][0-9]-*.sql | wc -l) draft"

aws ecr get-login-password --region us-east-1 \\
  | podman login --username AWS --password-stdin ${ACCOUNT}.dkr.ecr.us-east-1.amazonaws.com >/dev/null 2>&1
podman pull "$IMAGE" >/dev/null 2>&1 || { echo "BRIEF-R-PULL: FAIL exit=1"; exit 1; }
echo "BRIEF-R-PULL: PASS exit=0"

"\$STAGE/repo/scripts/contract-brief-r-on-rapid-admin.sh" "\$STAGE" "$IMAGE" "${RUN_ID}"
rc=\$?
echo "BRIEF-R-RUNNER: exit=\$rc"
exit \$rc
REMOTE
)

b64=$(printf '%s' "$script" | base64 | tr -d '\n')
params=$(mktemp "${TMPDIR:-/tmp}/brief-r-params.XXXXXX")
printf '{"commands":["echo %s | base64 -d | bash"]}' "$b64" > "$params"

echo
echo ">> running brief R acceptance on rapid-admin via SSM..."
...
rm -f "$params"
echo ">> command id: $cmd"
echo "$cmd" > "${TMPDIR:-/tmp}/brief-r-last-command-id"
...
if [ "$status" = "Success" ]; then
    ...
    echo "BRIEF-R-ACCEPTANCE-OK"
    exit 0
fi
echo "!! brief R acceptance FAILED (status $status)" >&2
