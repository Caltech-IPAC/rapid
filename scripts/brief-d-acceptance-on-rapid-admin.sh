#!/usr/bin/env bash
#
# brief-d-acceptance-on-rapid-admin.sh — stage this branch and the
# authoritative migration stream to rapid-admin, and run brief D's acceptance
# there via SSM.
#
...
#
# Usage: brief-d-acceptance-on-rapid-admin.sh [postgres-image-ref]
...
# `<rapid-parent>/rapid-wt-d`, so rapid_systems is one level up beside it —
...
RUN_ID="brief-d-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/brief-d.XXXXXX.tar.gz")
...
sha256sum -c "\$STAGE/manifest.sha256" || { echo "BRIEF-D-CHECKSUM: FAIL exit=1"; exit 1; }
echo "BRIEF-D-CHECKSUM: PASS exit=0"
...
echo "BRIEF-D-STREAM-REV: $STREAM_REV"
echo "BRIEF-D-MIGRATIONS: \$(ls \$STAGE/db-migrations/0[0-9][0-9]-*.sql | wc -l) stream, \$(ls \$STAGE/repo/migrations-draft/0[0-9][0-9]-*.sql | wc -l) draft"

aws ecr get-login-password --region us-east-1 \\
  | podman login --username AWS --password-stdin ${ACCOUNT}.dkr.ecr.us-east-1.amazonaws.com >/dev/null 2>&1
podman pull "$IMAGE" >/dev/null 2>&1 || { echo "BRIEF-D-PULL: FAIL exit=1"; exit 1; }
echo "BRIEF-D-PULL: PASS exit=0"

"\$STAGE/repo/scripts/contract-brief-d-on-rapid-admin.sh" "\$STAGE" "$IMAGE" "${RUN_ID}"
rc=\$?
echo "BRIEF-D-RUNNER: exit=\$rc"
exit \$rc
REMOTE
)

b64=$(printf '%s' "$script" | base64 | tr -d '\n')
params=$(mktemp "${TMPDIR:-/tmp}/brief-d-params.XXXXXX")
printf '{"commands":["echo %s | base64 -d | bash"]}' "$b64" > "$params"

echo
echo ">> running brief D acceptance on rapid-admin via SSM..."
...
rm -f "$params"
echo ">> command id: $cmd"
echo "$cmd" > "${TMPDIR:-/tmp}/brief-d-last-command-id"
...
if [ "$status" = "Success" ]; then
    ...
    echo "BRIEF-D-ACCEPTANCE-OK"
    exit 0
fi
echo "!! brief D acceptance FAILED (status $status)" >&2
