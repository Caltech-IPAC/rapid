#!/usr/bin/env bash
#
# brief-c-acceptance-on-rapid-admin.sh — stage this branch and the
# authoritative migration stream to rapid-admin, and run brief C's acceptance
# there via SSM.
#
...
#
# Usage: brief-c-acceptance-on-rapid-admin.sh [postgres-image-ref]
...
tarball=$(mktemp "${TMPDIR:-/tmp}/brief-c.XXXXXX.tar.gz")
...
sha256sum -c "\$STAGE/manifest.sha256" || { echo "BRIEF-C-CHECKSUM: FAIL exit=1"; exit 1; }
echo "BRIEF-C-CHECKSUM: PASS exit=0"
...
echo "BRIEF-C-STREAM-REV: $STREAM_REV"
echo "BRIEF-C-MIGRATIONS: \$(ls \$STAGE/db-migrations/0[0-9][0-9]-*.sql | wc -l) stream, \$(ls \$STAGE/repo/migrations-draft/0[0-9][0-9]-*.sql | wc -l) draft"

aws ecr get-login-password --region us-east-1 \\
  | podman login --username AWS --password-stdin ${ACCOUNT}.dkr.ecr.us-east-1.amazonaws.com >/dev/null 2>&1
podman pull "$IMAGE" >/dev/null 2>&1 || { echo "BRIEF-C-PULL: FAIL exit=1"; exit 1; }
echo "BRIEF-C-PULL: PASS exit=0"

"\$STAGE/repo/scripts/contract-brief-c-on-rapid-admin.sh" "\$STAGE" "$IMAGE" "${RUN_ID}"
rc=\$?
echo "BRIEF-C-RUNNER: exit=\$rc"
exit \$rc
REMOTE
)

b64=$(printf '%s' "$script" | base64 | tr -d '\n')
params=$(mktemp "${TMPDIR:-/tmp}/brief-c-params.XXXXXX")
printf '{"commands":["echo %s | base64 -d | bash"]}' "$b64" > "$params"

echo
echo ">> running brief C acceptance on rapid-admin via SSM..."
...
rm -f "$params"
echo ">> command id: $cmd"
echo "$cmd" > "${TMPDIR:-/tmp}/brief-c-last-command-id"
...
if [ "$status" = "Success" ]; then
    ...
    echo "BRIEF-C-ACCEPTANCE-OK"
    exit 0
fi
echo "!! brief C acceptance FAILED (status $status)" >&2
