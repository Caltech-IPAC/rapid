#!/usr/bin/env bash
#
# brief-g-acceptance-on-rapid-admin.sh — stage this branch and the
# authoritative migration stream to rapid-admin, and run brief G's acceptance
# there via SSM.
#
...
#
# Derived from `brief-c-acceptance-on-rapid-admin.sh`, which ran this exact
# staging path to a green acceptance — the only differences are the run-id
# prefix, the sentinel prefix, and the inner runner it invokes. Kept as a
# sibling rather than parameterized into one script because each brief's
# acceptance is a record of what was run for that brief, and a shared script
# edited in place stops being that record for the earlier ones.
#
...
#
# Usage: brief-g-acceptance-on-rapid-admin.sh [postgres-image-ref]
...
# `<rapid-parent>/wt-brief-g`, so rapid_systems is one level up beside it —
...
RUN_ID="brief-g-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/brief-g.XXXXXX.tar.gz")
...
sha256sum -c "\$STAGE/manifest.sha256" || { echo "BRIEF-G-CHECKSUM: FAIL exit=1"; exit 1; }
echo "BRIEF-G-CHECKSUM: PASS exit=0"
...
echo "BRIEF-G-STREAM-REV: $STREAM_REV"
echo "BRIEF-G-MIGRATIONS: \$(ls \$STAGE/db-migrations/0[0-9][0-9]-*.sql | wc -l) stream, \$(ls \$STAGE/repo/migrations-draft/0[0-9][0-9]-*.sql | wc -l) draft"

aws ecr get-login-password --region us-east-1 \\
  | podman login --username AWS --password-stdin ${ACCOUNT}.dkr.ecr.us-east-1.amazonaws.com >/dev/null 2>&1
podman pull "$IMAGE" >/dev/null 2>&1 || { echo "BRIEF-G-PULL: FAIL exit=1"; exit 1; }
echo "BRIEF-G-PULL: PASS exit=0"

"\$STAGE/repo/scripts/contract-brief-g-on-rapid-admin.sh" "\$STAGE" "$IMAGE" "${RUN_ID}"
rc=\$?
echo "BRIEF-G-RUNNER: exit=\$rc"
exit \$rc
REMOTE
)

b64=$(printf '%s' "$script" | base64 | tr -d '\n')
params=$(mktemp "${TMPDIR:-/tmp}/brief-g-params.XXXXXX")
printf '{"commands":["echo %s | base64 -d | bash"]}' "$b64" > "$params"

echo
echo ">> running brief G acceptance on rapid-admin via SSM..."
...
rm -f "$params"
echo ">> command id: $cmd"
echo "$cmd" > "${TMPDIR:-/tmp}/brief-g-last-command-id"
...
if [ "$status" = "Success" ]; then
    ...
    echo "BRIEF-G-ACCEPTANCE-OK"
    exit 0
fi
echo "!! brief G acceptance FAILED (status $status)" >&2
