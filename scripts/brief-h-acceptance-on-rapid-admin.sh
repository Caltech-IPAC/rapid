#!/usr/bin/env bash
#
# brief-h-acceptance-on-rapid-admin.sh — stage this branch and the
# authoritative migration stream to rapid-admin, and run brief H's acceptance
# there via SSM.
#
...
#
# Derived from `brief-e-acceptance-on-rapid-admin.sh`, which ran this exact
# staging path to a green acceptance — the only differences are the run-id
# prefix, the sentinel prefix, and the inner runner it invokes. E's is the
# template rather than an older brief's because E's inner runner carries the
# two fixes the earlier ones lack (criteria aggregation and the PASS2
# zero-skip gate) and this brief's twelve criteria are exactly the shape that
# made G's verdict lie. Kept as a sibling rather than parameterized into one
...
#
# Usage: brief-h-acceptance-on-rapid-admin.sh [postgres-image-ref]
...
# `<rapid-parent>/rapid-wt-h`, so rapid_systems is one level up beside it —
...
RUN_ID="brief-h-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/brief-h.XXXXXX.tar.gz")
...
#
# `.github` JOINS THE LIST for brief H. Acceptance 10 requires the workflow's
...
# the tree by extension trips over them — brief H's fields-reader scanner did
...
sha256sum -c "\$STAGE/manifest.sha256" || { echo "BRIEF-H-CHECKSUM: FAIL exit=1"; exit 1; }
echo "BRIEF-H-CHECKSUM: PASS exit=0"
...
echo "BRIEF-H-STREAM-REV: $STREAM_REV"
echo "BRIEF-H-MIGRATIONS: \$(ls \$STAGE/db-migrations/0[0-9][0-9]-*.sql | wc -l) stream, \$(ls \$STAGE/repo/migrations-draft/0[0-9][0-9]-*.sql | wc -l) draft"

aws ecr get-login-password --region us-east-1 \\
  | podman login --username AWS --password-stdin ${ACCOUNT}.dkr.ecr.us-east-1.amazonaws.com >/dev/null 2>&1
podman pull "$IMAGE" >/dev/null 2>&1 || { echo "BRIEF-H-PULL: FAIL exit=1"; exit 1; }
echo "BRIEF-H-PULL: PASS exit=0"

"\$STAGE/repo/scripts/contract-brief-h-on-rapid-admin.sh" "\$STAGE" "$IMAGE" "${RUN_ID}"
rc=\$?
echo "BRIEF-H-RUNNER: exit=\$rc"
exit \$rc
REMOTE
)

b64=$(printf '%s' "$script" | base64 | tr -d '\n')
params=$(mktemp "${TMPDIR:-/tmp}/brief-h-params.XXXXXX")
printf '{"commands":["echo %s | base64 -d | bash"]}' "$b64" > "$params"

echo
echo ">> running brief H acceptance on rapid-admin via SSM..."
...
rm -f "$params"
echo ">> command id: $cmd"
echo "$cmd" > "${TMPDIR:-/tmp}/brief-h-last-command-id"
...
if [ "$status" = "Success" ]; then
    ...
    echo "BRIEF-H-ACCEPTANCE-OK"
    exit 0
fi
echo "!! brief H acceptance FAILED (status $status)" >&2
