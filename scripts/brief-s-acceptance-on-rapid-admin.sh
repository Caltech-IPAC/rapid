#!/usr/bin/env bash
#
# brief-s-acceptance-on-rapid-admin.sh — stage this branch and the
# authoritative migration stream to rapid-admin, and run brief S's acceptance
# there via SSM.
#
...
#
# Derived from `brief-r-acceptance-on-rapid-admin.sh`'s staging path, which
# ran this exact shape to a green acceptance — the differences here are the
# run-id prefix, the sentinel prefix, the inner runner it invokes, and NO
# `migrations-draft/` staging: 044-052 were adopted into the authoritative
# `rapid_systems` stream on 2026-08-12 (`migrations-draft/README.md`), so
# there is nothing left in that directory but notes. Kept as a sibling
# rather than parameterized into one script because each brief's acceptance
# is a record of what was run for that brief, and a shared script edited in
# place stops being that record for the earlier ones.
#
...
#
# Usage: brief-s-acceptance-on-rapid-admin.sh [postgres-image-ref]
...
# `<rapid-parent>/rapid-wt-s`, so rapid_systems is one level up beside it —
...
RUN_ID="brief-s-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/brief-s.XXXXXX.tar.gz")
trap 'rm -f "$tarball"' EXIT

# The repo subset the suites need. Same exclusions as the operational runner
# (`sims/`, `RuBR/`, `c/` are not imported by either tier). `migrations-draft/`
# is staged too, even though it now holds only notes — cheap, and keeps this
# script's shape identical to its siblings rather than special-casing an
# empty directory.
#
# `.github` JOINS THE LIST, as it did for brief R: acceptance criterion R2's
# entry-point test reads the workflow file directly, and a staged tree
# without it fails on this host while passing everywhere the repo is checked
# out. `--exclude='._*'` as well as `--no-xattrs`: on this macOS `tar`, the
# flag stops xattrs being ARCHIVED but AppleDouble `._<name>` sidecars
# already on disk are ordinary files and ship anyway.
...
sha256sum -c "\$STAGE/manifest.sha256" || { echo "BRIEF-S-CHECKSUM: FAIL exit=1"; exit 1; }
echo "BRIEF-S-CHECKSUM: PASS exit=0"
...
echo "BRIEF-S-STREAM-REV: $STREAM_REV"
echo "BRIEF-S-MIGRATIONS: \$(ls \$STAGE/db-migrations/0[0-9][0-9]-*.sql | wc -l) stream migrations"

aws ecr get-login-password --region us-east-1 \\
  | podman login --username AWS --password-stdin ${ACCOUNT}.dkr.ecr.us-east-1.amazonaws.com >/dev/null 2>&1
podman pull "$IMAGE" >/dev/null 2>&1 || { echo "BRIEF-S-PULL: FAIL exit=1"; exit 1; }
echo "BRIEF-S-PULL: PASS exit=0"

"\$STAGE/repo/scripts/contract-brief-s-on-rapid-admin.sh" "\$STAGE" "$IMAGE" "${RUN_ID}"
rc=\$?
echo "BRIEF-S-RUNNER: exit=\$rc"
exit \$rc
REMOTE
)

b64=$(printf '%s' "$script" | base64 | tr -d '\n')
params=$(mktemp "${TMPDIR:-/tmp}/brief-s-params.XXXXXX")
printf '{"commands":["echo %s | base64 -d | bash"]}' "$b64" > "$params"

echo
echo ">> running brief S acceptance on rapid-admin via SSM..."
...
rm -f "$params"
echo ">> command id: $cmd"
echo "$cmd" > "${TMPDIR:-/tmp}/brief-s-last-command-id"
...
if [ "$status" = "Success" ]; then
    ...
    echo "BRIEF-S-ACCEPTANCE-OK"
    exit 0
fi
echo "!! brief S acceptance FAILED (status $status)" >&2
