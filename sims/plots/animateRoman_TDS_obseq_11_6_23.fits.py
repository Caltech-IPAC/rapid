...
def parseAndPlotData(filter):
    ...
    def next_set_of_obs(ax,my_ra,my_dec):

        for i in range(0,len(my_ra),4):
            ra_sca = []
            dec_sca = []

            ra_sca.append(my_ra[i])
            dec_sca.append(my_dec[i])

            ra_sca.append(my_ra[i+1])
            dec_sca.append(my_dec[i+1])

            ra_sca.append(my_ra[i+2])
            dec_sca.append(my_dec[i+2])

            ra_sca.append(my_ra[i+3])
            dec_sca.append(my_dec[i+3])

            ra_sca.append(my_ra[i])
            dec_sca.append(my_dec[i])
    
            ax.plot(ra_sca, dec_sca,"s", **marker_style3)
    ...
    for i in range(0,frames):
        ...
        next_set_of_obs(ax,my_ra,my_dec)

        plt.pause(pause_time)
        
        #plt.savefig(filter + "_footprints_" + str(i+1) + ".png")
