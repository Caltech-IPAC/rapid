#!/usr/bin/env python3
"""The finalize stage's fixture: prepare, run, check. ``make stage-finalize``.
...
finalize``, the same fixture running inside the pipeline image). The
fixture data this reads (``expected.json``, ``settings.toml``) is the
packaged copy under ``rapidpipe/selftest/fixtures/finalize/`` -- this
...
tool and declares no database access, so ``--tools`` changes only the
label (``register``'s handling of the finalized manifest shares the
difference-image path, ``tests/db/test_register_difference.py``).
"""
...
from rapidpipe.selftest.finalize import FIXTURE  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"stage-finalize: tools={args.tools} "
    ...
    try:
        ...
        print(f"stage-finalize: {exc}", file=sys.stderr)
        return 1

    for failure in result.checks.failures:
        print(f"stage-finalize: FAIL {failure}")
    verdict = "PASS" if not result.checks.failures else "FAIL"
    print(f"stage-finalize: {verdict} ({result.checks.passed} checks passed, "
