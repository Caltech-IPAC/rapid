#!/usr/bin/env python3
"""The prune stage's fixture: prepare, run, check. ``make stage-prune``.
...
prune``, the same fixture running inside the pipeline image). The fixture
data this reads (``expected.json``, ``settings.toml``) is the packaged
copy under ``rapidpipe/selftest/fixtures/prune/`` -- this directory holds
only this thin reference, not a second copy.

Exit 0 when every check passes, 1 otherwise. The same stage against
PostgreSQL (the child tables, the not-best exclusion, the `prunedmerges`
rows, provenance) is ``tests/db/test_prune.py``.
"""
...
from rapidpipe.selftest.prune import FIXTURE  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"stage-prune: database=fake "
    ...
    try:
        ...
        print(f"stage-prune: {exc}", file=sys.stderr)
        return 1

    for failure in result.checks.failures:
        print(f"stage-prune: FAIL {failure}")
    verdict = "PASS" if not result.checks.failures else "FAIL"
    print(f"stage-prune: {verdict} ({result.checks.passed} checks passed, "
