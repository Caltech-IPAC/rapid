#!/usr/bin/env python3
"""The statistics stage's fixture: prepare, run, check. ``make stage-statistics``.
...
statistics``, the same fixture running inside the pipeline image). The
fixture data this reads (``expected.json``, ``settings.toml``) is the
packaged copy under ``rapidpipe/selftest/fixtures/statistics/`` -- this
...
PostgreSQL is ``tests/db/test_statistics.py``.
"""
...
from rapidpipe.selftest.statistics import FIXTURE  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"stage-statistics: database=fake "
    ...
    try:
        ...
        print(f"stage-statistics: {exc}", file=sys.stderr)
        return 1

    for failure in result.checks.failures:
        print(f"stage-statistics: FAIL {failure}")
    verdict = "PASS" if not result.checks.failures else "FAIL"
    print(f"stage-statistics: {verdict} ({result.checks.passed} checks passed, "
