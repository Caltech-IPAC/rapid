#!/usr/bin/env python3
"""The maintain stage's fixture: prepare, run, check. ``make stage-maintain``.
...
maintain``, the same fixture running inside the pipeline image). The
fixture data this reads (``expected.json``) is the packaged copy under
``rapidpipe/selftest/fixtures/maintain/`` -- this directory holds only
this thin reference, not a second copy.

Exit 0 when every check passes, 1 otherwise. The same stage against
PostgreSQL (the child-table CLUSTER/ANALYZE for real) is
``tests/db/test_maintain.py``.
"""
...
from rapidpipe.selftest.maintain import FIXTURE  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"stage-maintain: database=fake "
    ...
    try:
        ...
        print(f"stage-maintain: {exc}", file=sys.stderr)
        return 1

    for failure in result.checks.failures:
        print(f"stage-maintain: FAIL {failure}")
    verdict = "PASS" if not result.checks.failures else "FAIL"
    print(f"stage-maintain: {verdict} ({result.checks.passed} checks passed, "
