...
difference``, the same fixture running inside the pipeline image). The
fixture data this reads (``expected.json``, ``settings.toml``) is the
packaged copy under ``rapidpipe/selftest/fixtures/difference/`` -- this
...
from rapidpipe.selftest.difference import FIXTURE  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"stage-difference: tools={args.tools} "
    ...
    try:
        ...
        print(f"stage-difference: {exc}", file=sys.stderr)
        return 1
    ...
    print(f"stage-difference: {verdict} ({result.checks.passed} checks passed, "
