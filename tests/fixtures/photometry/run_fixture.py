...
photometry``, the same fixture running inside the pipeline image). The
fixture data this reads (``expected.json``, ``settings.toml``) is the
packaged copy under ``rapidpipe/selftest/fixtures/photometry/`` -- this
...
tool and opens no database connection, so ``--tools`` changes only the
label.
"""
...
from rapidpipe.selftest.photometry import FIXTURE  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"stage-photometry: tools={args.tools} "
    ...
    try:
        ...
        print(f"stage-photometry: {exc}", file=sys.stderr)
        return 1

    for failure in result.checks.failures:
        print(f"stage-photometry: FAIL {failure}")
    verdict = "PASS" if not result.checks.failures else "FAIL"
    print(f"stage-photometry: {verdict} ({result.checks.passed} checks passed, "
