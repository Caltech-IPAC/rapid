#!/usr/bin/env bash
# run-fixd-schema-probe-on-rapid-db.sh — drive the FixD schema probe against rapid-db.
#
...
#
# Usage: run-fixd-schema-probe-on-rapid-db.sh [image-ref]
...
RUN_ID="fixd-schema-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/fixd-schema.XXXXXX.tar.gz")
...
aws s3 cp "$tarball" "s3://$BUCKET/$PREFIX/fixd-schema.tar.gz" --only-show-errors
...
aws s3 cp "s3://$BUCKET/$PREFIX/fixd-schema.tar.gz" "\$STAGE/fixd-schema.tar.gz" --only-show-errors
echo "$sha  \$STAGE/fixd-schema.tar.gz" > "\$STAGE/manifest.sha256"
sha256sum -c "\$STAGE/manifest.sha256" || { echo "!! checksum FAILED" >&2; exit 1; }
mkdir -p "\$STAGE/repo"
tar xzf "\$STAGE/fixd-schema.tar.gz" -C "\$STAGE/repo"
...
podman run --rm --entrypoint="" -v "\$STAGE/repo":/fixd:Z -w /fixd \\
  ...
  --network host \\
  "$IMAGE" python3.11 -m submission.test.live_fixd_schema_probe 2>&1 | head -400
live=\${PIPESTATUS[0]}
...
params=$(mktemp "${TMPDIR:-/tmp}/fixd-schema-params.XXXXXX")
