#!/usr/bin/env bash
# run-fixc-schema-probe-on-rapid-db.sh — drive the FixC schema probe against rapid-db.
#
...
#
# Usage: run-fixc-schema-probe-on-rapid-db.sh [image-ref]
...
RUN_ID="fixc-schema-$(date -u +%Y%m%dT%H%M%SZ)"
...
tarball=$(mktemp "${TMPDIR:-/tmp}/fixc-schema.XXXXXX.tar.gz")
...
aws s3 cp "$tarball" "s3://$BUCKET/$PREFIX/fixc-schema.tar.gz" --only-show-errors
...
aws s3 cp "s3://$BUCKET/$PREFIX/fixc-schema.tar.gz" "\$STAGE/fixc-schema.tar.gz" --only-show-errors
echo "$sha  \$STAGE/fixc-schema.tar.gz" > "\$STAGE/manifest.sha256"
sha256sum -c "\$STAGE/manifest.sha256" || { echo "!! checksum FAILED" >&2; exit 1; }
mkdir -p "\$STAGE/repo"
tar xzf "\$STAGE/fixc-schema.tar.gz" -C "\$STAGE/repo"
...
podman run --rm --entrypoint="" -v "\$STAGE/repo":/fixc:Z -w /fixc \\
  ...
  --network host \\
  "$IMAGE" python3.11 -m submission.test.live_fixc_schema_probe 2>&1 | head -400
live=\${PIPESTATUS[0]}
...
params=$(mktemp "${TMPDIR:-/tmp}/fixc-schema-params.XXXXXX")
