...
* Reference images are assumed to exist for all observed sky tiles and filters (upper bound on difference-image volume)
* Only ZOGY positive difference-image products are delivered to MAST; SFFT, Naive, and negative images are excluded
...
* Sample file delivery to MAST is required 6+ months before the first data release *(TBD)*
...
A forced-photometry backend and the ``cforcepsfaper`` C module had been developed as of February 2026. There is demand for a similar service elsewhere within Roman, and collaboration on a shared tool is under discussion.
